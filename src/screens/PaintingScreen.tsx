...
const PaintingScreen = ({ route }: PaintingScreenProps) => {
  ...
  const initializeTrack = () => {
    const track = {
      ...
      artist: paintingDetails?.artist ? paintingDetails?.artist as string : "",
  ...
  const headerAnimatedStyle = useAnimatedStyle(() => {
    const display = interpolate(scrollOfset.value > IMG_HEIGHT ? scrollOfset.value : 0, [0, IMG_HEIGHT], [0, 1]);
        ...
        />
        {
          isLoading ? (
            <View style={[styles.paintingTitle,{alignSelf: 'center'}]}>
              <SkeletonView height={height * 0.028} width={width * 0.6} borderRadius={20} />
            </View>
          ) : (
            <Text style={[styles.paintingTitle, { fontSize: height * 0.028, paddingHorizontal: 50 }]}>{paintingDetails?.title}</Text>
          ...
          <View style={{ alignItems: 'center', gap: 10 }}>
            {
              isLoading ? (
                <View style={styles.paintingTitle}>
                  <SkeletonView height={height * 0.028} width={width * 0.8} borderRadius={20} />
                </View>
              ) : (
                <Text style={[styles.paintingTitle, { fontSize: height * 0.028 }]}>{paintingDetails?.title}</Text>
            ...
            {
              isLoading ? (
                <SkeletonView width={width * 0.95} height={150} borderRadius={20} />
              ) : (
